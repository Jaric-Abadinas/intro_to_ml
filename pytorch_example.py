...
# Initialize wandb
wandb.init(project="mnist-pytorch", entity="abadin48")
...
# Reload MNIST dataset with normilization
print("Reloading MNIST dataset with normilization...")
...
for epoch in range (num_epochs):
    ...
    for i, data in pbar:
        # Unpack the current batch of datat into inputs and labels
        inputs, labels = data
        ...
        # Forward pass: compute the model's predicted outputs match the true labels.
        outputs = model(inputs)

        # Compute the loss value: this measures how well the predicted outputs match the true labels.
        loss = criterion(outputs, labels)

        # Backward pass: compute the gradient of the loss with
        loss.backward()
